...
If you already have a working ruby environment, the easiest way to install is just do:
...
* *rack*
  ...
  * Additionally, you need a rack handler, one of the following is suggested:
    * *unicorn*
      ...
      * Use Apache, Nginx or similar front-ends for virtualhosts, SSL and such.
...
==== 3.2.2. Configuration
When starting up RSence for the first time, a configuration wizard is run. Just press return on the questions to use the default configuration. The default option is captilized: pressing return at a "Y/n" prompt will select "Y"


==== 3.3.3. Open a web browser
* By defalt, the RSence listens on port 8001
...
Starting in forefround mode with debug and logging in foreground:
  bin/run -d --log-fg
...
* In development mode (see 4.6.), plugins are (re/un)loaded when:
  ...
  * The plugin's ruby or yaml files are made.
